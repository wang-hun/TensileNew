...
修改此逻辑时要特别注意：只能删除本次探测新增的额外 IP，不能删除或修改用户原有 IP；不要同时给多个网卡保留同一设备网段地址；提权探测失败、用户取消 UAC、设置失败和所有端口失败都需要给出可理解的提示。网络检查是否成功的判据必须是：添加候选额外 IP 后，使用该本地 IP 绑定 TCP 客户端连接 PLC 的 Modbus TCP 端口 502 成功；不能用未绑定本地 IP 的系统路由连接作为成功兜底，避免探测结果和主程序实际连接走不同网卡。网络探测问题应限制在探测流程、网络配置和主窗口重连流程内处理，不应修改 `DeltaPLC2` 等底层 PLC 通讯封装的连接接口。探测成功返回主进程后，主程序应先复用或确认已有 PLC 连接，不能无条件断开已有连接再重连；只有用户手动点击“重新连接”这类明确重连操作才应强制断开重建连接。

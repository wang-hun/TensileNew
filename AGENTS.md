...
修改此逻辑时要特别注意：只能删除本次探测新增的额外 IP，不能删除或修改用户原有 IP；不要同时给多个网卡保留同一设备网段地址；提权探测失败、用户取消 UAC、设置失败和所有端口失败都需要给出可理解的提示。网络检查是否成功的判据是：添加候选额外 IP 后，重试连接 PLC 的 Modbus TCP 端口 502；优先使用该本地 IP 绑定 TCP 客户端连接，若绑定连接失败但系统路由下未绑定连接已经成功，也视为本次网络检查成功。探测成功返回主进程后，主程序应先复用或确认已有 PLC 连接，不能无条件断开已有连接再重连；只有用户手动点击“重新连接”这类明确重连操作才应强制断开重建连接。
